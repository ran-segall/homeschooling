...
import type { Lesson, LessonStep, Subject } from '@/lib/database.types'
        ...
        {words.map((w, i) =>
          /\s/.test(w) ? w : (
            <span
              ...
              style={{ cursor: 'pointer', borderBottom: '1px dotted var(--ink3)', paddingBottom: 1 }}
            >
              {w}
            </span>
          )
...
function StepMultipleChoice({ content, onNext }: { content: Record<string, unknown>; onAnswer: (correct: boolean) => void; onNext: (answer: unknown, correct: boolean) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <p style={{ fontSize: 18, fontWeight: 500, color: 'var(--ink)', lineHeight: 1.5, margin: 0 }}>
      ...
      {!checked
        ? <NextButton onClick={handleCheck} label="Check" disabled={selected === null} />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map((item, i) => (
          <div key={item} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ width: 24, height: 24, borderRadius: '50%', background: 'var(--ink)', color: '#fff', fontSize: 12, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {i + 1}
            </span>
            <div style={{ flex: 1, padding: '12px 16px', background: 'var(--card)', border: '1px solid var(--line)', borderRadius: 10, fontSize: 15, color: 'var(--ink)' }}>
              {item}
            </div>
      ...
      {checked && (
        <div style={{ padding: '14px 18px', borderRadius: 12, background: isCorrect ? 'var(--lime-bg)' : '#fef2f2', border: `1px solid ${isCorrect ? 'var(--lime)' : '#fecaca'}` }}>
          ...
          {!isCorrect && originalItems.map((item, i) => (
            <p key={i} style={{ margin: '4px 0 0', fontSize: 13, color: '#dc2626' }}>{i + 1}. {item}</p>
          ))}
        </div>
      )}
      {!checked
...
function StepDragToMatch({ content, onNext }: { content: Record<string, unknown>; onNext: (answer: unknown, correct: boolean) => void }) {
  ...
  const rightIndexes = useRef(rightItems)
  ...
  const correctMatches = pairs.filter((p, i) => {
    const matchedRightIdx = matches[i]
    return matchedRightIdx !== undefined && rightIndexes.current[matchedRightIdx] === p[1]
  }).length
          ...
          {pairs.map((p, i) => (
            <button
              ...
              style={{
                ...
                color: selected === i ? '#fff' : 'var(--ink)',
                fontSize: 14, cursor: checked ? 'default' : 'pointer', textAlign: 'left',
                fontFamily: 'var(--font-body)',
                transition: 'all 120ms',
              }}
            >
          ...
          {rightIndexes.current.map((item, j) => {
            ...
            return (
              <button
                ...
                disabled={checked || (matchedBy !== undefined && selected === null)}
                style={{
                  padding: '12px 14px',
                  borderRadius: 10,
                  ...
                  fontSize: 14, cursor: checked ? 'default' : 'pointer', textAlign: 'left', color: 'var(--ink)',
                  fontFamily: 'var(--font-body)',
                  transition: 'all 120ms',
                }}
              >
...
function StepInteractive({ content, onNext }: { content: Record<string, unknown>; onNext: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <p style={{ fontSize: 18, lineHeight: 1.7, color: 'var(--ink)', margin: 0 }}>
      ...
      <div
        ...
        style={{
          ...
          border: `1px solid ${interacted ? 'var(--violet)' : 'var(--line)'}`,
          borderRadius: 16,
          padding: '32px 24px',
          textAlign: 'center',
          cursor: 'pointer',
      ...
      >
        {interacted ? (
          ...
          <>
            ...
            <p style={{ margin: 0, fontSize: 15, color: 'var(--ink2)' }}>Tap to explore this idea</p>
          </>
        )}
      </div>
...
function StepWrittenAnswer({ content, kid, onNext }: { content: Record<string, unknown>; kid: { name: string; age: number | null }; onNext: (answer: unknown, correct: boolean) => void }) {
  ...
  async function getAIFeedback() {
    ...
    try {
      const res = await fetch('/api/ai-feedback', {
        ...
        body: JSON.stringify({ question: content.question, answer, kidName: kid.name, kidAge: kid.age }),
      })
      const data = await res.json()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {!!content.note && (
        <p style={{ fontSize: 13, color: 'var(--ink3)', margin: '-12px 0 0' }}>{String(content.note)}</p>
      )}
      <textarea
        value={answer}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <textarea
        ...
        placeholder="What do you think? There's no wrong answer…"
        rows={4}
        style={{
          width: '100%',
          padding: '14px 16px',
...
export default function LessonPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadLesson() {
    const [lessonRes, stepsRes] = await Promise.all([
      supabase.from('lessons').select('*, subject:subjects(*)').eq('id', id).single(),
      supabase.from('lesson_steps').select('*').eq('lesson_id', id).order('step_order'),
    ])
    if (!lessonRes.data) { router.replace('/home'); return }
    setLesson(lessonRes.data as Lesson & { subject?: Subject })
    setSteps(stepsRes.data as LessonStep[] ?? [])

    // Create session
    const { data: session } = await supabase
      .from('lesson_sessions')
      .insert({ lesson_id: id, kid_id: kid!.id })
      .select('id')
      .single()
  ...
  async function handleStepComplete(answer: unknown, isCorrect: boolean) {
    if (sessionId && steps[currentStep]) {
      await supabase.from('step_responses').insert({
    ...
    if (currentStep < steps.length - 1) {
      ...
      // All steps done — close session and go to feedback
        ...
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, height: 56 }}>
            <button
              ...
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 20, padding: 0, lineHeight: 1 }}
            >
              ←
            </button>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '40px 24px 80px' }}>
        ...
        {currentStep === 0 && (
          <div style={{ marginBottom: 36 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              {lesson.subject?.label}
            </span>
            <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 36, fontWeight: 400, color: 'var(--ink)', margin: '6px 0 0', lineHeight: 1.2 }}>
              {lesson.title}
            </h1>
          </div>
        )}
        ...
        {/* Step content */}
        {step?.type === 'Read' && (
          <StepRead content={step.content} onNext={() => handleStepComplete(null, true)} />
        )}
        ...
        {step?.type === 'Drag to order' && (
          <StepDragToOrder content={step.content} onNext={handleStepComplete} />
        )}
        {step?.type === 'Drag to match' && (
          <StepDragToMatch content={step.content} onNext={handleStepComplete} />
        )}
        {step?.type === 'Interactive' && (
          <StepInteractive content={step.content} onNext={() => handleStepComplete(null, true)} />
        )}
        {step?.type === 'Written answer' && (
          <StepWrittenAnswer
            content={step.content}
            kid={{ name: kid.name, age: kid.age }}
            onNext={handleStepComplete}
          />
        )}
        {step?.type === 'Socratic' && (
          <StepSocratic content={step.content} onNext={handleStepComplete} />
        )}
      </div>
    </div>
  )
